...
var STORAGE_TYPE = ENUM_STORAGE_TYPE.OSS;
...
function getAccessUrl(file_key) {
    ...
    if (STORAGE_TYPE == ENUM_STORAGE_TYPE.S3) {
        return S3MinioUpload.init(S3Conf.Config).getAccessUrl(file_key);
    } else if (STORAGE_TYPE == ENUM_STORAGE_TYPE.OSS) {
        return 'http://cdn.workaioss.tiegushi.com/' + file_key;
    } else {
        console.log('STORAGE_TYPE err' + STORAGE_TYPE);
        return '';
    }
...
function putFile(file_key, localFile, cb) {
    if (STORAGE_TYPE == ENUM_STORAGE_TYPE.S3) {
        S3MinioUpload.init(S3Conf.Config).uploadFile(file_key, localFile, cb);
    } else if (STORAGE_TYPE == ENUM_STORAGE_TYPE.OSS) {
        putFileAliyun(file_key, localFile, cb);
    } else {
        console.log('STORAGE_TYPE err' + STORAGE_TYPE);
    }
