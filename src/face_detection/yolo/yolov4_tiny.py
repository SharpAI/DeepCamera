...
from yolo_visualization import BBoxVisualization

try:
    ctypes.cdll.LoadLibrary('./yolo/plugins/libyolo_layer.so')
except OSError as e:
    print('ERROR: failed to load ./yolo/plugins/libyolo_layer.so.  '
                     'Did you forget to do a "make" in the "./yolo/plugins/" '
                     'subdirectory?')
    exit()

class YoloModel:
    def __init__(self):
        self.default_model_dir = './yolo'
        self.default_model = 'yolov4-tiny-416'
        self.default_labels = 'coco.names'
        ...
        self.parser.add_argument('--threshold', type=float, default=0.70,
        ...
        self.cls_dict = self.load_labels(self.args.labels)
        self.vis = BBoxVisualization(self.cls_dict)
        self.h, self.w = get_input_shape(self.args.model)
        self.trt_yolo = TrtYOLO(self.default_model_dir,self.args.model, (self.h, self.w), self.args.category_num)

        self.labels = self.load_labels(self.args.labels)
        print('labels {}'.format(self.labels))
    def draw_bboxes(self, img, objects):
        gun_count = 0
        """Draw detected bounding boxes on the original image."""
        for obj in objects:
            #logging.debug('obj is {}'.format(obj))
            cl = int(obj['class_id'])
            if cl == 1:
                gun_count += 1
            cf = obj['confidence']
            x_min, y_min, x_max, y_max = int(obj['xmin']), int(obj['ymin']), int(obj['xmax']), int(obj['ymax'])
            color = self.vis.colors[cl]
            cv2.rectangle(img, (x_min, y_min), (x_max, y_max), color, 2)
            txt_loc = (max(x_min+2, 0), max(y_min+2, 0))
            txt = '{} {:.2f}'.format(cl, cf)
            img = draw_boxed_text(img, txt, txt_loc, color)
        return (img,gun_count)
...
def get_input_shape_by_model_name(model_name):
    ...
    print('yolo input shape is {}x{}'.format(h,w))
    return h, w
def get_input_shape(model_name):
    """Infer input tensor shape fixed by 416.
    # Args
        model_name
    """
    return 416, 416
...
class TrtYOLO(object):
    ...
    def _load_engine(self):
        TRTbin = '{}/{}.trt'.format(self.model_path,self.model)
        with open(TRTbin, 'rb') as f, trt.Runtime(self.trt_logger) as runtime:
            return runtime.deserialize_cuda_engine(f.read())

    def __init__(self,model_path ,model, input_shape, category_num=2,
                 cuda_ctx=None):
    ...
    def detect(self, img, conf_th=0.3, letter_box=None):
        ...
        boxes, scores, classes = _postprocess_yolo(
            ...
            nms_threshold=0.25, input_shape=self.input_shape,
