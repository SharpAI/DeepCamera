#!/bin/bash
export DEVICE_UUID_FILEPATH=/dev/ro_serialno
export DEVICE_GROUP_ID_FILEPATH=/data/usr/com.deep.workai/cache/groupid.txt
...
if [ -f upload_api-v2_human.py ]; then
  #WORKER_TYPE=detect python2 upload_api-v2_human.py worker --loglevel INFO -E -n detect -c 1 -Q detect &
  WORKER_TYPE=embedding python2 upload_api-v2_human.py worker --loglevel INFO -E -n embedding -c 1 -Q embedding &
elif [ -f upload_api-v2_human.pyc ]; then
  WORKER_TYPE=embedding python2 upload_api-v2_human.pyc worker --loglevel INFO -E -n embedding -c 1 -Q embedding &
else
