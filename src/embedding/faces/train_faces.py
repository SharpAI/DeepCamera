...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    category_number = len(result)
    global how_many_training_steps
    how_many_training_steps = (category_number) * 500
...
def train_from_bottlenecks(image_dir, bottleneck_dir):
    ...
    if test_accuracy < 0.85:
        print('Accuracy is too low, not save pb file.')
        return
    if print_misclassified_test_images:
        print('=== MISCLASSIFIED TEST IMAGES ===')
        for i, test_filename in enumerate(test_filenames):
...
if __name__ == '__main__':
    groupid = sys.argv[1]
    style = sys.argv[2]
    group_path = os.path.join(BASEPATH, groupid, style)
    image_dir = os.path.join(group_path, 'face_dataset')
    embedding_dir = os.path.join(group_path, "face_embedding")
    output_graph = os.path.join(group_path, 'embedding_graph.pb')
    output_labels = os.path.join(group_path, 'output_labels.txt')

    if not os.path.exists(group_path):
        os.mkdir(group_path)
        os.mkdir(image_dir)
        os.mkdir(embedding_dir)
    train_from_bottlenecks(image_dir=image_dir, bottleneck_dir=embedding_dir)
